...
function renderUsers() {
    ...
    paginatedUsers.forEach(u => {
        ...
        <tr>
            <td>${u.id}</td>
            <td>${u.full_name || 'غير معروف'}</td>
            <td>${u.phone || 'غير معروف'}</td>
            <td><strong>${u.username}</strong></td>
            <td>${getRoleName(u.role_id)}</td>
            <td>${getStatusBadge(u.status)}</td>
            <td>
                <button class="btn-action view" onclick="viewUserDetails(${u.id})">
                    <i class="bi bi-eye"></i>
                </button>

                ${u.status !== 'rejected'
                ? `<button class="btn-action block" onclick="updateUserStatus(${u.id}, 'rejected')">
    ...
    if (pageInfo) {
        pageInfo.textContent =
            `صفحة ${usersCurrentPage} من ${Math.ceil(filteredUsers.length / usersPerPage) || 1}`;
    }
...
function changeUsersPage(step) {
    ...
    const filteredCount = usersData.length;
    const maxPages = Math.ceil(filteredCount / usersPerPage);

    usersCurrentPage += step;

    if (usersCurrentPage < 1) usersCurrentPage = 1;
    if (usersCurrentPage > maxPages) usersCurrentPage = maxPages;
...
const ABSENCE_API = 'https://msaraty.ddns.net/api/absence-requests';

let absenceRequestsData = [];

async function fetchAbsenceRequests() {
    try {
        const res = await fetch(ABSENCE_API);
        if (!res.ok) throw new Error("فشل جلب طلبات الغياب");
        absenceRequestsData = await res.json();
    } catch (error) {
        console.error("fetchAbsenceRequests Error:", error);
        absenceRequestsData = [];
    }
    await fetchUsers();
    renderAbsenceRequestsNotifications();
...
function getAbsenceTypeLabel44(type) {
    if (type === "pickup") return "غياب ذهاب";
    if (type === "dropoff") return "غياب عودة";
    if (type === "full") return "غياب كامل";
    return "غير محدد";
}

function getAbsenceTypeColor44(type) {
    if (type === "pickup") return "#0d6efd";
    if (type === "dropoff") return "#fd7e14";
    if (type === "full") return "#dc3545";
    return "#6c757d";
}

function formatAbsenceDate44(dateString) {
...
function getArabicDayName44(dateString) {
    if (!dateString) return "غير محدد";
    ...
    return dayMap[date.getDay()] || "غير محدد";
}

function renderAbsenceRequestsNotifications() {
    const container = document.getElementById("absence-container");
    if (!container) return;

    const typeFilter = document.getElementById("absence-filter-type44")?.value || "all";
    const dayFilter = document.getElementById("absence-filter-day44")?.value || "all";
    const searchText = (document.getElementById("absence-search44")?.value || "").trim().toLowerCase();

    container.innerHTML = "";

    if (!Array.isArray(absenceRequestsData) || absenceRequestsData.length === 0) {
        container.innerHTML = `<div class="empty-state44">لا توجد طلبات غياب حالياً</div>`;
        return;
    }

    let filteredRequests = [...absenceRequestsData]
        .sort((a, b) => {
        ...
        .filter(req => {
            const student = studentsData.find(s => Number(s.id) === Number(req.student_id));
            const studentName = (student?.name || "").toLowerCase();
            const universityNumber = String(student?.university_number || "").toLowerCase();
            const dayName = getArabicDayName44(req.date);

            const matchType = typeFilter === "all" || req.type === typeFilter;
            const matchDay = dayFilter === "all" || dayName === dayFilter;
            const matchSearch =
                !searchText ||
                studentName.includes(searchText) ||
                universityNumber.includes(searchText);

            return matchType && matchDay && matchSearch;
        });

    if (filteredRequests.length === 0) {
        container.innerHTML = `<div class="empty-state44">لا توجد نتائج مطابقة</div>`;
        return;
    }

    container.innerHTML = filteredRequests.map(req => {
        const student = studentsData.find(s => Number(s.id) === Number(req.student_id));
        const studentName = student?.name || `طالب رقم ${req.student_id}`;
        const typeLabel = getAbsenceTypeLabel44(req.type);
        const typeColor = getAbsenceTypeColor44(req.type);
        const dayName = getArabicDayName44(req.date);
        const formattedDate = formatAbsenceDate44(req.date);

        return `
        <div class="absence-card44">
            <div class="absence-top-bar44" style="background: linear-gradient(90deg, ${typeColor}, ${typeColor}99);"></div>

            <div class="absence-card-body44">

                <div class="absence-card-header44">
                    <div>
                        <h4 class="absence-student-name44">${studentName}</h4>
                        <div class="absence-date-line44">
                            <span><i class="bi bi-calendar-event"></i> ${formattedDate}</span>
                            <span><i class="bi bi-calendar-week"></i> ${dayName}</span>
                        </div>
                    </div>

                    <span class="absence-type-badge44"
                          style="background:${typeColor}18; color:${typeColor}; border:1px solid ${typeColor}44;">
                        ${typeLabel}
                    </span>
                </div>

                <div class="absence-card-footer44">
                    <span class="absence-request-id44">طلب رقم: ${req.id ?? "-"}</span>

                    <button class="absence-details-btn44" onclick="openAbsenceDetails44(${req.id})">
                        عرض التفاصيل
                    </button>
                </div>
...
function openAbsenceDetails44(requestId) {
    const req = absenceRequestsData.find(r => Number(r.id) === Number(requestId));
    if (!req) return;
    ...
    const pickupStationName =
        stationsData.find(st => Number(st.id) === Number(student?.pickup_station_id))?.station_name || "غير محدد";

    const dropoffStationName =
        stationsData.find(st => Number(st.id) === Number(student?.dropoff_station_id))?.station_name || "غير محدد";

    const universityName =
        universitiesData.find(u => Number(u.id) === Number(student?.university_id))?.university_name || "غير محدد";

    const collegeName =
        collegesData.find(c => Number(c.id) === Number(student?.college_id))?.college_name || "غير محدد";

    const departmentName =
        departmentsData.find(d => Number(d.id) === Number(student?.department_id))?.department_name || "غير محدد";

    const levelName =
        levelsData.find(l => Number(l.id) === Number(student?.level_id))?.level_name || "غير محدد";

    const modal = document.getElementById("absence-details-modal44");
    if (!modal) return;

    modal.innerHTML = `
    <div class="modal-overlay44" onclick="closeAbsenceDetails44()">

        <div class="absence-modal-card44" onclick="event.stopPropagation()">

            <div class="absence-modal-top44"
                 style="background: linear-gradient(90deg, ${typeColor}, ${typeColor}99);">
            </div>

            <div class="absence-modal-content44">

                <h2 class="absence-modal-title44">تفاصيل طلب الغياب</h2>

                <div class="absence-modal-student-bar44">
                    ${student?.name || `طالب رقم ${req.student_id}`}
                </div>

                <div class="absence-info-grid44">

                    <div class="absence-info-box44">
                ...
                <h3 class="absence-section-title44">بيانات الطالب</h3>

                <div class="absence-info-grid44">

                    <div class="absence-info-box44">
                        <label>الاسم</label>
                        <div>${student?.name || "غير محدد"}</div>
                    </div>

                    <div class="absence-info-box44">
                        <label>الهاتف</label>
                        <div>${student?.phone || "غير متوفر"}</div>
                    </div>

                    <div class="absence-info-box44">
                        <label>الرقم الجامعي</label>
                        <div>${student?.university_number || "غير متوفر"}</div>
                    </div>

                    <div class="absence-info-box44">
                        <label>المدينة</label>
                        <div>${student?.city || "غير محدد"}</div>
                    </div>

                    <div class="absence-info-box44">
                        <label>الجنس</label>
                        <div>${student?.gender || "غير محدد"}</div>
                    </div>

                    <div class="absence-info-box44">
                        <label>الحالة</label>
                        <div>${student?.state || "غير محدد"}</div>
                    </div>
                    ...
                    <div class="absence-info-box44">
                        <label>محطة الصعود</label>
                        <div>${pickupStationName}</div>
                    </div>

                    <div class="absence-info-box44">
                        <label>محطة النزول</label>
                        <div>${dropoffStationName}</div>
                    </div>

                </div>

                <div class="absence-modal-actions44">
                    <button class="absence-close-btn44" onclick="closeAbsenceDetails44()">
                        إغلاق
                    </button>
                </div>
...
function closeAbsenceDetails44() {
    const modal = document.getElementById("absence-details-modal44");
    if (!modal) return;
    modal.style.display = "none";
    modal.innerHTML = "";
...
function ensureTripAutoDistributionPanelExists() {
    const studentsSection = document.querySelector(".trip-students-section99");
    const toolsBar = document.querySelector(".trip-students-tools99");

    if (!studentsSection || !toolsBar) return;
    if (getEl("trip-auto-distribution-box")) return;

    const wrapper = document.createElement("div");
    wrapper.id = "trip-auto-distribution-box";
    wrapper.className = "trip-auto-distribution-box99";
    ...
    wrapper.innerHTML = `
        <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:10px;flex-wrap:wrap;margin-bottom:10px;">
            <div>
                <strong style="display:block;margin-bottom:4px;">التوزيع التلقائي</strong>
                <div class="hint-text">
                    يحدد الطلاب تلقائيًا داخل الرحلة الحالية فقط حسب نقطة النزول والجنس وسعة الباص، مع بقاء حق التعديل اليدوي للأدمن.
                </div>
            </div>
        </div>
        ...
        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(190px,1fr));gap:10px;">
            <div>
                <label style="display:block;margin-bottom:6px;">نقطة النزول المستهدفة</label>
                <select id="trip-auto-dropoff-filter" class="filter-select"></select>
            </div>

            <div>
                <label style="display:block;margin-bottom:6px;">وضع التوزيع حسب الجنس</label>
                <select id="trip-auto-gender-mode" class="filter-select">
                    <option value="none">بدون فصل</option>
                    <option value="priority_first">تجميع حسب الأولوية أولاً</option>
                    <option value="separate_max">فصل قدر الإمكان</option>
                </select>
            </div>

            <div>
                <label style="display:block;margin-bottom:6px;">الأولوية للجنس</label>
                <select id="trip-auto-gender-priority" class="filter-select">
                    <option value="auto">تلقائي حسب العدد الأكبر</option>
                    <option value="female">الإناث أولاً</option>
                    <option value="male">الذكور أولاً</option>
                </select>
            </div>

            <div>
                <label style="display:block;margin-bottom:6px;">خيار إضافي</label>
                <label style="display:flex;align-items:center;gap:8px;border:1px solid #e5e7eb;border-radius:10px;padding:11px 12px;">
                    <input id="trip-auto-allow-mixed" type="checkbox" checked>
                    <span>السماح بالإكمال من الجنس الآخر عند الحاجة</span>
                </label>
            </div>
        </div>

        <div style="display:flex;gap:10px;flex-wrap:wrap;margin-top:12px;">
            <button type="button" class="btn btn-primary" onclick="runTripAutoDistribution()">
                تنفيذ التوزيع التلقائي
            </button>

            <button type="button" class="btn btn-secondary" onclick="clearTripAutoDistributionSelection()">
                مسح التحديد التلقائي
            </button>
        </div>

        <div id="trip-auto-distribution-summary" class="hint-text" style="margin-top:10px;">
            اختر إعدادات التوزيع ثم اضغط تنفيذ التوزيع التلقائي.
        </div>
    `;

    toolsBar.parentNode.insertBefore(wrapper, toolsBar);
}

function resetTripAutoDistributionControls() {
    const dropoffSelect = getEl("trip-auto-dropoff-filter");
    const genderMode = getEl("trip-auto-gender-mode");
    const genderPriority = getEl("trip-auto-gender-priority");
    const allowMixed = getEl("trip-auto-allow-mixed");
    const summary = getEl("trip-auto-distribution-summary");

    if (dropoffSelect) dropoffSelect.value = "all";
    if (genderMode) genderMode.value = "none";
    if (genderPriority) genderPriority.value = "auto";
    if (allowMixed) allowMixed.checked = true;

    if (summary) {
        summary.innerHTML = "اختر إعدادات التوزيع ثم اضغط تنفيذ التوزيع التلقائي.";
    }
}

function getStudentsForAutoDistributionPool() {
    const tripData = collectTripFormData(false);
    if (!tripData) return [];

    const selectedAssign = getAssignById(tripData.assign_id);
    ...
    studentsData.forEach(student => {
        ...
        if (dayFilterMode === "trip_day_only" && !studentMatchesTripDay(student, tripData.trip_date)) {
            return;
        }

        if (!studentMatchesAbsenceFilter(student, tripData.trip_date)) {
            return;
        }
...
function populateTripAutoDropoffOptions() {
    const select = getEl("trip-auto-dropoff-filter");
    if (!select) return;

    const currentValue = select.value || "all";
    const pool = getStudentsForAutoDistributionPool();

    const uniqueDropoffs = [...new Set(
        pool
            .map(student => Number(student.dropoff_station_id))
            .filter(id => !isNaN(id) && id > 0)
    )];

    select.innerHTML = `<option value="all">كل نقاط النزول</option>`;

    uniqueDropoffs.forEach(stationId => {
        select.innerHTML += `
            <option value="station_${stationId}">
                نقطة نزول: ${getStationName(stationId)}
            </option>
        `;
    });
...
function syncTripAutoDistributionUI() {
    ensureTripAutoDistributionPanelExists();
    populateTripAutoDropoffOptions();
}

function getTripAutoPriorityGender(priorityValue, femalesCount, malesCount) {
    if (priorityValue === "female") return "female";
    if (priorityValue === "male") return "male";

    return femalesCount >= malesCount ? "female" : "male";
}

function sortStudentsForAutoDistribution(students) {
    return [...students].sort((a, b) => {
        const stationA = getStationName(a.dropoff_station_id);
        const stationB = getStationName(b.dropoff_station_id);

        const stationCompare = stationA.localeCompare(stationB, "ar");
...
function getTripAutoTargetDropoffLabel() {
    const select = getEl("trip-auto-dropoff-filter");
    if (!select) return "كل نقاط النزول";

    const option = select.options[select.selectedIndex];
    return option ? option.textContent.trim() : "كل نقاط النزول";
}

function updateTripAutoDistributionSummary(selectedStudents = null, fullPool = null) {
    const summary = getEl("trip-auto-distribution-summary");
    if (!summary) return;
    ...
    const femaleCount = selectedStudents.filter(student => normalizeGenderValue(student.gender) === "female").length;
    const maleCount = selectedStudents.filter(student => normalizeGenderValue(student.gender) === "male").length;
    const unknownCount = selectedStudents.length - femaleCount - maleCount;

    const capacity = getBusCapacity(getBusById(getEl("trip-bus-select")?.value || ""));
    const remainingSeats = Math.max(capacity - selectedStudents.length, 0);
    const poolCount = Array.isArray(fullPool) ? fullPool.length : selectedStudents.length;

    const targetLabel = getTripAutoTargetDropoffLabel();

    summary.innerHTML = `
        تم التحديد التلقائي لـ <strong>${selectedStudents.length}</strong> طالب/طالبة
        من أصل <strong>${poolCount}</strong> طالب/طالبة مطابقين للشروط،
        مع استهداف <strong>${targetLabel}</strong>.
        <br>
        إناث: <strong>${femaleCount}</strong>
        — ذكور: <strong>${maleCount}</strong>
        ${unknownCount ? `— غير محدد: <strong>${unknownCount}</strong>` : ""}
        — المقاعد المتبقية: <strong>${remainingSeats}</strong>
    `;
}

function clearTripAutoDistributionSelection() {
    ...
    updateTripAutoDistributionSummary([]);
    updateTripSeatsCounter();
    updateTripModalPreview();
    renderTripStudentsList();
}

function runTripAutoDistribution() {
    const tripData = collectTripFormData();
    if (!tripData) return;

    const selectedBus = getBusById(tripData.bus_id);
    const capacity = getBusCapacity(selectedBus);

    if (!capacity || capacity <= 0) {
        alert("سعة الباص غير صالحة للتوزيع التلقائي");
        return;
    }

    let pool = getStudentsForAutoDistributionPool();

    const targetDropoffValue = getEl("trip-auto-dropoff-filter")?.value || "all";
    const genderMode = getEl("trip-auto-gender-mode")?.value || "none";
    const genderPriority = getEl("trip-auto-gender-priority")?.value || "auto";
    const allowMixed = !!getEl("trip-auto-allow-mixed")?.checked;

    if (String(targetDropoffValue).startsWith("station_")) {
        const stationId = Number(String(targetDropoffValue).replace("station_", ""));
        pool = pool.filter(student => Number(student.dropoff_station_id) === stationId);
    }

    pool = sortStudentsForAutoDistribution(pool);

    if (!pool.length) {
        alert("لا يوجد طلاب مطابقون لشروط التوزيع التلقائي الحالية");
        return;
    }
    ...
    if (genderMode === "none") {
        selectedStudents = pool.slice(0, capacity);
    } else {
        const females = pool.filter(student => normalizeGenderValue(student.gender) === "female");
        const males = pool.filter(student => normalizeGenderValue(student.gender) === "male");
        const unknown = pool.filter(student => {
            const gender = normalizeGenderValue(student.gender);
            return gender !== "female" && gender !== "male";
        });

        const primaryGender = getTripAutoPriorityGender(genderPriority, females.length, males.length);

        let primaryGroup = primaryGender === "female" ? females : males;
        let secondaryGroup = primaryGender === "female" ? males : females;

        if (genderMode === "priority_first") {
            selectedStudents = [...primaryGroup];

            if (allowMixed && selectedStudents.length < capacity) {
                selectedStudents = selectedStudents.concat(secondaryGroup);
            }

            if (allowMixed && selectedStudents.length < capacity) {
                selectedStudents = selectedStudents.concat(unknown);
            }

            selectedStudents = selectedStudents.slice(0, capacity);
        } else if (genderMode === "separate_max") {
            selectedStudents = primaryGroup.slice(0, capacity);

            if (allowMixed && selectedStudents.length < capacity) {
                const rest = [...secondaryGroup, ...unknown];
                selectedStudents = selectedStudents.concat(
                    rest.slice(0, capacity - selectedStudents.length)
                );
            }
        }
    }
    ...
    if (!selectedStudents.length) {
        alert("تعذر تكوين توزيع تلقائي وفق الإعدادات الحالية");
        return;
    }

    tripSelectedStudentIds = selectedStudents.map(student => Number(student.id));
    ...
    updateTripAutoDistributionSummary(selectedStudents, pool);
    updateTripSeatsCounter();
    updateTripModalPreview();
    renderTripStudentsList();
}
...
function resetTripModal() {
    ...
    resetTripAutoDistributionControls();
    syncTripAutoDistributionUI();
}
...
function resolveTripData(trip) {
    ...
    const createdByName = creator
        ? (creator.full_name && creator.full_name.trim() !== ""
            ? creator.full_name
            : creator.username)
        : "غير محدد";
        ...
        ? `
            ...
            return `
                                <tr>
                                  ...
                                  <td>
    <span class="trip-status-badge ${trip.status === "completed"
                    ? "status-completed"
                    : trip.status === "pending"
                        ? "status-pending"
...
function openTripDetailsModal(tripId) {
   ...
   <div class="trip-detail-card99">
    ...
    <strong>
        ${resolvedTrip.status === "completed"
            ? "مكتملة"
            : resolvedTrip.status === "pending"
                ? "قيد الانتظار"
                : resolvedTrip.status === "approved"
                    ? "معتمدة"
                    : "غير محددة"
        }
    </strong>
...
function openPaymentsTab(tabId, btnElement) {

    const parent = document.getElementById('sec-payments');
    if (!parent) return;


    parent.querySelectorAll('.payments-tab-content')
        .forEach(content => {
...
async function fetchStudentPayments() {
    try {
        const res = await fetch("https://msaraty.ddns.net/api/show-student-payments");
        const data = await res.json();

        studentPaymentsData = Array.isArray(data) ? data : [];
...
function renderStudentPayments() {
    ...
    const body = document.getElementById("student-payments-body");
    if (!body) return;

    const searchQuery = (document.getElementById("student-payments-search")?.value || "").toLowerCase();
    const statusFilter = document.getElementById("student-payments-filter")?.value || "all";

    body.innerHTML = "";

    let filtered = (studentPaymentsData || []).filter(p => {
        ...
        <tr>
            ...
            <td>
                ...
                <button class="btn-action view"
                    onclick="openPaymentDetailsById(${p.id})"
                    title="عرض التفاصيل">
                    <i class="bi bi-eye"></i>
                </button>

                <button class="btn-action edit" title="تعديل">
                    <i class="bi bi-pencil-square"></i>
                </button>
...
window.openPaymentDetailsById = function (id) {

    const payment = (studentPaymentsData || []).find(p => p.id == id);
    ...
    const imageHTML = payment.receipt_image
        ? `<img src="${payment.receipt_image}" class="payment-image">`
    ...
    modal.innerHTML = `
        <div class="payment-modal-box">
            ...
            <!-- CLOSE -->
            <button class="close-modal btn btn-danger" onclick="closePaymentDetails()">×</button>
            ...
            <div class="payment-details">
                ...
                <div class="payment-row"><strong>اسم الطالب:</strong> ${student?.name || "-"}</div>
                <div class="payment-row"><strong>رقم الجوال:</strong> ${student?.phone || "-"}</div>

                <div class="payment-divider"></div>

                <div class="payment-row"><strong>المبلغ:</strong> ${payment.amount}</div>
                <div class="payment-row"><strong>الحالة:</strong> ${payment.status}</div>
                <div class="payment-row"><strong>الشهر:</strong> ${payment.for_month}</div>
                <div class="payment-row"><strong>البنك:</strong> ${payment.bank_name || "-"}</div>
                <div class="payment-row"><strong>رقم الإيصال:</strong> ${payment.receipt_number || "-"}</div>
                <div class="payment-row"><strong>السبب:</strong> ${payment.rejection_reason || "لا يوجد"}</div>
                <div class="payment-row"><strong>التاريخ:</strong> ${new Date(payment.created_at).toLocaleString("ar")}</div>
            ...
            <div class="payment-actions">
                ...
                <button class="btn btn-success btn-success-color"
...
function rejectPayment(id) {
    ...
    overlay.innerHTML = `
        <div class="reject-box">
            ...
            <h3>رفض عملية الدفع</h3>

            <label>سبب الرفض</label>
            <textarea id="reject-reason-input" placeholder="اكتب سبب رفض العملية..."></textarea>

            <div class="reject-actions">

                <button class="btn btn-danger"
                    onclick="submitRejectPayment(${id})">
                    تأكيد الرفض
                </button>
...
function submitRejectPayment(id) {
...
function closeRejectModal() {
    const el = document.querySelector(".reject-overlay");
    if (el) el.remove();
}
...
let driverPaymentsData = [];

async function fetchDriverPayments() {
    try {
        const res = await fetch(`http://localhost:3000/show-driver-payments`);
        const data = await res.json();

        driverPaymentsData = Array.isArray(data) ? data : [];

        renderDriverPayments();
    } catch (err) {
        console.error("Driver Payments Error:", err);
    }
}

function renderDriverPayments() {

    const body = document.getElementById("driver-payments-body");
    if (!body) return;

    const searchQuery =
        (document.getElementById("driver-payments-search")?.value || "").toLowerCase();

    const statusFilter =
        document.getElementById("driver-payments-filter")?.value || "all";

    body.innerHTML = "";

    let filtered = driverPaymentsData.filter(p => {

        const driver = driversData.find(d => d.id == p.driver_id);

        const name = (driver?.name || driver?.name_driver || "").toLowerCase();

        const matchesSearch =
            name.includes(searchQuery) ||
            String(p.driver_id).includes(searchQuery);
    ...
    filtered.forEach(p => {
        ...
        <tr>
            ...
            <td>${driver?.name || driver?.name_driver || "غير معروف"}</td>
            ...
            <td>
                ...
                <button class="btn-action view" title="عرض"
    onclick="openDriverPaymentDetailsById(${p.id})">
    <i class="bi bi-eye"></i>
</button>
...
window.openDriverPaymentDetailsById = function (id) {

    const payment = (driverPaymentsData || []).find(p => p.id == id);

    if (!payment) {
        console.error("Driver Payment not found:", id);
        return;
    }

    openDriverPaymentDetails(payment);
};

function openDriverPaymentDetails(payment) {
    ...
    modal.innerHTML = `
        <div class="payment-modal-box">
            ...
            <!-- CLOSE -->
            <button class="close-modal btn btn-danger"
                onclick="closeDriverPaymentDetails()">×</button>

           

            <!-- DETAILS ONLY (NO ACTIONS) -->
            <div class="payment-details">

                <div class="payment-row"><strong>اسم السائق:</strong> ${driver?.name || driver?.name_driver || "-"}</div>

                <div class="payment-row"><strong>رقم الجوال:</strong> ${driver?.phone || "-"}</div>

                <div class="payment-divider"></div>

                <div class="payment-row"><strong>المبلغ:</strong> ${payment.amount}</div>

                <div class="payment-row"><strong>الحالة:</strong>
                    ${payment.status === "paid"
            ? `<span class="badge bg-success">مدفوع</span>`
            : `<span class="badge bg-danger">غير مدفوع</span>`
        }
                ...
                <div class="payment-row"><strong>ID العملية:</strong> ${payment.id}</div>

                <div class="payment-row"><strong>ID السائق:</strong> ${payment.driver_id}</div>
...
function closeDriverPaymentDetails() {
